...
SELECT * 
,ROW_NUMBER() OVER(ORDER By Sales DESC) AS Row_Number_by_Sales
,RANK() OVER(ORDER BY sales DESC) AS Rank_By_Sales
,DENSE_RANK() OVER(ORDER BY sales DESC) AS Dense_Rank_By_Sales
FROM Sales_Data
...
SELECT * 
,RANK() OVER(PARTITION BY product_id ORDER BY Sales DESC) AS Rank_Within_Each_Product
,DENSE_RANK() OVER(PARTITION BY product_id ORDER BY Sales DESC) AS Dense_Rank_Within_Each_Products
FROM Sales_Data